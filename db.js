...
const mockClient = {
  query: async (text, params) => {
    ...
    if (text.includes('INSERT INTO message_content')) {
      // Store encrypted content and attachments
      const [token, encrypted_content, attachmentsJson] = params;
      if (!inMemoryMessages[token]) inMemoryMessages[token] = {};
      inMemoryMessages[token].encrypted_content = encrypted_content.toString('base64');
      if (attachmentsJson) {
        try {
          inMemoryMessages[token].attachments = JSON.parse(attachmentsJson);
        } catch (e) {
          inMemoryMessages[token].attachments = [];
        }
      } else {
        inMemoryMessages[token].attachments = [];
      }
    ...
    if (text.includes('SELECT encrypted_content') && text.includes('FROM message_content') && text.includes('WHERE token = $1')) {
      ...
      if (entry && entry.encrypted_content) {
        return Promise.resolve({
          rows: [{ encrypted_content: Buffer.from(entry.encrypted_content, 'base64'), attachments: entry.attachments || [] }],
          rowCount: 1,
        });
      } else {
        return Promise.resolve({ rows: [], rowCount: 0 });
