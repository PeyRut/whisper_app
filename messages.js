...
router.post('/', 
    ...
    async (req, res) => {
        ...
        const { encrypted_content, expiration_duration_minutes, is_one_time_view, attachments = [] } = req.body;
        ...
        try {
            ...
            const insertContentQuery = `
                INSERT INTO message_content (token, encrypted_content, attachments, created_at)
                VALUES ($1, $2, $3, NOW());
            `;
            const encryptedContentBuffer = Buffer.from(encrypted_content, 'base64');
            // Store attachments as JSON string
            const attachmentsJson = JSON.stringify(attachments);
            await client.query(insertContentQuery, [token, encryptedContentBuffer, attachmentsJson]);
